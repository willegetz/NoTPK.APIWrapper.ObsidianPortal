...
namespace APIWrapper.ObsidianPortal.Tests
{
    ...
    public class RequestHelper_Tests
    {
        ...
        {
            Dictionary<string, string> optionalParams = GetOptinalParts();
        ...
        [TestMethod]
        public void TestUpdateAuthorizationParts()
        {
            var result = GetUpdatedAuthParts();
            Approvals.VerifyAll(result);
        }
        ...
        {
            var updatedAuthParts = GetUpdatedAuthParts();
            var result = RequestHelpers.BuildAuthorizationHeader(updatedAuthParts);
            Approvals.Verify(result);
        }

        private static SortedDictionary<string, string> GetAuthParts()
        {
        ...
        private static Dictionary<string, string> GetOptinalParts()
        {
        ...
        private static SortedDictionary<string, string> GetUpdatedAuthParts()
        {
            var authorizationParts = GetAuthParts();

            var optionalParams = GetOptinalParts();

            var signature = @"Lxbp/sGTkhBpCFjtlTL7P218iug=";
            return RequestHelpers.UpdateAuthorizationParts(authorizationParts, optionalParams, signature);
        }
    }
}